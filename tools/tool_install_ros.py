...
ros_mirror_dic = {
    "tsinghua": {
        "ROS1": "http://mirrors.tuna.tsinghua.edu.cn/ros/ubuntu/",
        "ROS2": "http://mirrors.tuna.tsinghua.edu.cn/ros2/ubuntu/",
    },
    "mirrorz": {
        "ROS1": "http://mirrors.cernet.edu.cn/ros/ubuntu/",
        "ROS2": "http://mirrors.cernet.edu.cn/ros2/ubuntu",
    },
    "ustc": {
        "ROS1": "https://mirrors.ustc.edu.cn/ros/ubuntu/",
        "ROS2": "https://mirrors.ustc.edu.cn/ros2/ubuntu/",
    },
    "huawei": {
        "ROS1": "https://repo.huaweicloud.com/ros/ubuntu/",
        "ROS2": "https://repo.huaweicloud.com/ros2/ubuntu/",
    },
    "packages.ros": {
        "ROS1": "http://packages.ros.org/ros/ubuntu/",
        "ROS2": "http://packages.ros.org/ros2/ubuntu/",
    },
}
...
class Tool(BaseTool):
    ...
    def get_mirror_by_code(self, code, arch="amd64", first_choose="tsinghua"):
        ...
        # armhf架构，优先使用官方源
        if arch == "armhf":
            ros2_choose_queue = ["packages.ros", "tsinghua", "ustc", "huawei"]
    ...
    def select_mirror(self):
        ...
        if codename in ros_dist_dic.keys() or codename in ros2_dist_dic.keys():
            if "ustc" in ros_dist_dic.get(codename, []) or "ustc" in ros2_dist_dic.get(
                codename, []
            ):
                supported_mirrors.append("ustc")

        if codename in ros_dist_dic.keys() or codename in ros2_dist_dic.keys():
            if "tsinghua" in ros_dist_dic.get(
                codename, []
            ) or "tsinghua" in ros2_dist_dic.get(codename, []):
                supported_mirrors.append("tsinghua")

        if codename in ros_dist_dic.keys() or codename in ros2_dist_dic.keys():
            if "huawei" in ros_dist_dic.get(
                codename, []
            ) or "huawei" in ros2_dist_dic.get(codename, []):
                supported_mirrors.append("huawei")

        # 添加 mirrorz 镜像源支持
        if codename in ros_dist_dic.keys() or codename in ros2_dist_dic.keys():
            if "mirrorz" in ros_dist_dic.get(
                codename, []
    ...
    def add_source(self):
        ...
        # add source 1
        mirrors = self.get_mirror_by_code(
            osversion.get_codename(), arch=arch, first_choose=selected_mirror
        )
        ...
        while retry_mirror != selected_mirror:
            ...
            mirrors = self.get_mirror_by_code(
                osversion.get_codename(), arch=arch, first_choose=retry_mirror
            )
            PrintUtils.print_info(
                "根据您的系统和选择，为您推荐安装源为{}".format(mirrors)
            )
            source_data = ""
            for mirror in mirrors:
                source_data += "deb [arch={}]  {} {} main\n".format(
            ...
            PrintUtils.print_error(
                "您选择了相同的镜像源，四次换源后都失败了，请及时联系小鱼获取解决方案并处理！"
            )
    ...
    def add_key(self):
        ...
        has_apt_key = CmdTask("command -v apt-key", 10).run()[0] == 0
        if has_apt_key:
            cmd_result = CmdTask(
                "curl -s {} | sudo apt-key add -".format(key_url)
            ).run()
            if cmd_result[0] != 0:
                cmd_result = CmdTask(
                    "curl -s {} | sudo apt-key add -".format(key_url)
            ...
            PrintUtils.print_info(
                tr.tr("检测到系统未提供apt-key，切换为gpg keyring导入方式...")
            )
            cmd_result = CmdTask(
                "curl -fsSL {} | sudo gpg --dearmor -o /etc/apt/trusted.gpg.d/ros.gpg".format(
                    key_url
                )
            ).run()
            if cmd_result[0] == 0:
                CmdTask("sudo chmod 644 /etc/apt/trusted.gpg.d/ros.gpg", 10).run()

        # 针对trusted.gpg.d问题解决方案
        if FileUtils.check_result(cmd_result, ["trusted.gpg.d"]):
            cmd_result = CmdTask(
                "curl -s {} | sudo gpg --no-default-keyring --keyring gnupg-ring:/etc/apt/trusted.gpg.d/ros.gpg --import".format(
                    key_url
                )
            ).run()
            cmd_result = CmdTask(
                "sudo chmod 644 /etc/apt/trusted.gpg.d/ros.gpg", 10
            ).run()
    ...
    def support_install(self):
        # check support
        if (osversion.get_codename() not in ros_dist_dic.keys()) and (
            osversion.get_codename() not in ros2_dist_dic.keys()
        ):
            PrintUtils.print_error(
                "小鱼:检测当前系统{}{}:{} 暂不支持一键安装ROS,请关注公众号《鱼香ROS》获取帮助.".format(
                    osversion.get_name(),
                    osversion.get_version(),
                    osversion.get_codename(),
                )
            )
            return False
        PrintUtils.print_success(
            "小鱼:检测当前系统{}{}:{} 支持一键安装ROS".format(
                osversion.get_name(), osversion.get_version(), osversion.get_codename()
            )
        )
        return True
    ...
    def choose_and_install_ros(self):
        ...
        if code == 2:
            # 第一次尝试
            cmd_result = CmdTask(
                "sudo {} install  {} -y".format(
                    install_tool_apt, dic_base[install_version]
                ),
                300,
                os_command=True,
            ).run()
            ...
            if FileUtils.check_result(
                cmd_result,
                ["未满足的依赖关系", "unmet dependencies", "but it is not installable"],
            ):
                # 尝试使用aptitude解决依赖问题
                PrintUtils.print_warn(
                    "============================================================"
                ...
                cmd_result = CmdTask(
                    "sudo {} install   {} ".format(
                        install_tool, dic_base[install_version]
                    ),
                    300,
                    os_command=True,
                ).run()
        ...
        # ======================================================桌面版本==============================================================
            ...
            cmd_result = CmdTask(
                "sudo {} install   {} -y".format(
                    install_tool_apt, RosVersions.get_desktop_version(install_version)
                ),
                300,
                os_command=True,
            ).run()
            ...
            if FileUtils.check_result(
                ...
                PrintUtils.print_delay(
                    "请注意我，检测你在安装过程中出现依赖问题，请在稍后输入n,再选择y,即可解决（若无法解决，清在稍后手动运行命令: sudo aptitude install {})".format(
                        RosVersions.get_desktop_version(install_version)
                    )
                )
                import time
                ...
                cmd_result = CmdTask(
                    "sudo {} install   {}".format(
                        install_tool, RosVersions.get_desktop_version(install_version)
                    ),
                    300,
                    os_command=True,
                ).run()
        ...
        if cmd_result[0] != 0:
            if FileUtils.check_result(cmd_result, ["--fix-broken install"]):
                ...
                if code == 2:
                    ...
                    cmd_result = CmdTask(
                        "sudo {} install   {} -y".format(install_tool, rosname),
                        300,
                        os_command=False,
                    ).run()
